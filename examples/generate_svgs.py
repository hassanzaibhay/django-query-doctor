...
create_terminal_svg(
    ...
    lines=[
        ...
        {"text": "Location: myapp/views.py:27 in book_list", "color": "#808080", "indent": 3},
        {"text": "Code: Book.objects.filter(title=books[0].title).count()", "color": "#808080", "indent": 3},
        {"text": "Fix: Assign the queryset result to a variable and reuse it instead of executing the same query multiple times", "color": "#4EC9B0", "indent": 3},
        {"text": "Queries: 2 | Est. savings: ~0.0ms", "color": "#808080", "indent": 3},
        {"text": ""},
        {"text": "INFO: Missing index: column \"title\" on Book (table \"testapp_book\") is used in WHERE/ORDER BY but has no index", "color": "#569CD6", "bold": True},
        {"text": "Location: myapp/views.py:27 in book_list", "color": "#808080", "indent": 3},
        {"text": "Fix: Add to Book's Meta.indexes: indexes = [models.Index(fields=[\"title\"], name=\"idx_testapp_book_title\")]", "color": "#4EC9B0", "indent": 3},
        {"text": ""},
        {"text": "INFO: Fat SELECT: 8 columns from \"testapp_book\" including large fields: description", "color": "#569CD6", "bold": True},
        {"text": "Location: myapp/views.py:24 in book_list", "color": "#808080", "indent": 3},
        {"text": "Fix: Use .defer('description') to skip loading large fields, or .values()/.values_list() if you don't need model instances", "color": "#4EC9B0", "indent": 3},
    ],
)
...
# Content captured from a real v2.1.0 QueryFixer dry run (generate_diff plus
...
create_terminal_svg(
    ...
    lines=[
        {"text": ""},
        {"text": "Dry run: showing proposed changes:", "color": "#D4D4D4"},
        {"text": ""},
        {"text": "--- myapp/views.py", "color": "#F44747"},
        {"text": "+++ myapp/views.py", "color": "#28CA41"},
        {"text": "@@ -2,1 +2,1 @@", "color": "#569CD6"},
        {"text": "  [N+1 detected: 12 queries for table \"testapp_author\" (via Book.author)] [MANUAL FIX ONLY]", "color": "#DCDCAA"},
        {"text": "-     books = Book.objects.all()", "color": "#F44747"},
        {"text": "+     books = Book.objects.all().select_related('author')", "color": "#28CA41"},
        {"text": "--- myapp/views.py", "color": "#F44747"},
        {"text": "+++ myapp/views.py", "color": "#28CA41"},
        {"text": "@@ -5,1 +5,1 @@", "color": "#569CD6"},
        {"text": "  [Inefficient queryset evaluation: len(qs)]", "color": "#DCDCAA"},
        {"text": "-     total = len(books)", "color": "#F44747"},
        {"text": "+     total = books.count()", "color": "#28CA41"},
        {"text": "--- myapp/views.py", "color": "#F44747"},
        {"text": "+++ myapp/views.py", "color": "#28CA41"},
        {"text": "@@ -6,1 +6,1 @@", "color": "#569CD6"},
        {"text": "  [Missing index: column \"published_date\" on Book (table \"testapp_book\") is used in WHERE/ORDER BY but has no index]", "color": "#DCDCAA"},
        {"text": "-     recent = Book.objects.filter(published_date__gte=cutoff)", "color": "#F44747"},
        {"text": "+     # TODO: Consider adding an index via Meta.indexes - Add to Book's Meta.indexes: indexes = [models.Index(fields=[\"published_date\"], name=\"idx_testapp_book_published_date\")]", "color": "#28CA41"},
        {"text": "+     recent = Book.objects.filter(published_date__gte=cutoff)", "color": "#28CA41"},
        {"text": ""},
        {"text": "3 fix(es) available. Run with --apply to write changes.", "color": "#DCDCAA", "bold": True},
        {"text": "([MANUAL FIX ONLY] fixes are shown here but refused by --apply)", "color": "#808080"},
    ],
)
