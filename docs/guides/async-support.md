...
- **`async with diagnose_queries()`**: raises -- it is a synchronous `@contextmanager` and does not implement the async protocol. **The exception type depends on your Python version**: `TypeError` on 3.11 and later, `AttributeError: __aenter__` on 3.10, which is CPython's own change and not something this package controls. Do not write `except TypeError` around it if you support 3.10. Same test class, which asserts both.
